...
class VqaDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        qst_cap = [qst_vocab.word2idx(w) for w in vqa[idx]['question_tokens']]
        ...
        if load_ans:
            ...
            numericalized_caption = np.array([qst_vocab.word2idx('<pad>')] * 300)
            numericalized_caption[0]=vocab.stoi["<SOS>"]

            for i in range(1,len(vqa[idx]['caption_tokens'][caption_idx])):
                for w in vqa[idx]['caption_tokens'][caption_idx]:
                    if w in vocab.stoi:
                        input = vocab.stoi[w]
                    else:
                        input=vocab.stoi["<UNK>"]
                    numericalized_caption[i] = input


            sample['caption'] = numericalized_caption
            qst_cap_arr = np.array([qst_vocab.word2idx('<pad>')] * 350)
            # arr = np.concatenate((qst_cap,numericalized_caption),axis=0)
            qst_cap+=numericalized_caption.tolist()
            qst_cap_arr[:len(qst_cap)] = [i for i in qst_cap]

            # if len(qst_cap_arr) <94:
            #     for i in range(len(qst_cap_arr),94):
            #         qst_cap_arr[i] = 0

            sample['qstcap'] = torch.tensor(qst_cap_arr)
...
def get_loader(input_dir, input_vqa_train, input_vqa_valid, max_qst_length, max_cap_length, max_num_ans, batch_size,
               num_workers):
    transform = {
