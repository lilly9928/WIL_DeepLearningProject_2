...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(data):
    qc_ids, ac_ids, question,image = data
    qa_nodes = set(qc_ids) | set(ac_ids)
    extra_nodes = set()
    for qid in qa_nodes:
        for aid in qa_nodes:
            if qid != aid and qid in cpnet_simple.nodes and aid in cpnet_simple.nodes:
                extra_nodes |= set(cpnet_simple[qid]) & set(cpnet_simple[aid])
    extra_nodes = extra_nodes - qa_nodes
    return (sorted(qc_ids), sorted(ac_ids), question, sorted(extra_nodes),image)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
    qc_ids, ac_ids, question, extra_nodes,image = data
    cid2score = get_LM_score(qc_ids+ac_ids+extra_nodes, question)
    icid2score = get_LM_score(qc_ids + ac_ids + extra_nodes, image)

    return (qc_ids, ac_ids, question, extra_nodes, cid2score,icid2score)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part3(data):
    qc_ids, ac_ids, question, extra_nodes, cid2score , icid2score = data
    ...
    i2qmask = arange_img < len(qc_ids)
    i2amask = (arange_img >= len(qc_ids)) & (arange_img < (len(qc_ids) + len(ac_ids)))
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    with open(grounded_path, 'r', encoding='utf-8') as fin_ground, open(statement_path, 'r', encoding='utf-8') as fin_state:
        ...
        for j, line in enumerate(lines_ground):
            ...
            Imgcontext ="{}.".format(statement_obj['image_caption'])
            qa_data.append((q_ids, a_ids, QAcontext,Imgcontext))

    with Pool(num_processes) as p:
        res1 = list(tqdm(p.imap(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1, qa_data), total=len(qa_data)))

    res2 = []
    for j, _data in enumerate(tqdm(res1)):
        # if j % 100 == 0: print (j)
...
# if __name__ == '__main__':
    ...
    # generate_adj_matrices_from_grounded_concepts('/data2/KJE/RSG/data/csqa/grounded/train.grounded.jsonl',
    #                                              '/data2/KJE/RSG/data/cpnet/conceptnet.en.pruned.graph',
    #                                              '/data2/KJE/RSG/data/cpnet/concept.txt',
    #                                              'data2/KJE/RSG//tmp/asdf', 40)
