...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        with open(os.path.join(os.path.dirname(VCR_DATALOADER_DIR), 'cocoontology.json'), 'r') as f:
            coco = json.load(f)
...
if __name__ == '__main__':
    ...
    input_file = '/data2/KJE/VCR/statement/test.statement.jsonl'
    output_file = '/data2/KJE/VCR/statement/test_n.statement.jsonl'
    ...
    for index, item in tqdm(enumerate(data), total=len(data), desc="Processing data"):
        ...
        data_id =int(''.join(filter(str.isdigit, item["id"])))
        change_id = int(''.join(filter(str.isdigit, test[index][1]['metadata']['annot_id'])))
        if data_id ==change_id:
            # 원하는 값으로 변경
            item["question"]["stem"] = test[index][1]['metadata']['question_sents'][0]
            for choices_index, choices in enumerate(item["question"]["choices"]):
                choices["text"] = test[index][1]['metadata']['answer_sents'][choices_index]

            with open(os.path.join(VCR_IMAGES_DIR, test[index]['metadata_fn']), 'r') as f:
                metadata = json.load(f)
