...
def create_matcher_patterns(cpnet_vocab_path, output_path, debug=False):
    ...
    for doc in tqdm(docs, total=len(cpnet_vocab)):
        ...
        #conceptnet의 단어를 패턴으로 등록
        pattern = create_pattern(nlp, doc, debug)
        #return pattern :token.Lamma_
        if debug:
            if not pattern[0]:
                f.write(pattern[1] + '\n')
...
def ground_mentioned_concepts(nlp, matcher, s, ans=None):
    ...
    if ans is not None:
        ...
        ans_words = nlp(ans)
        # print(ans_words)
        patterns = [{'TEXT': token.text.lower()} for token in ans_words]
        ans_matcher.add(ans, [patterns])
...
#input :res({"sent": s, "ans": a, "qc": question_concepts, "ac": answer_concepts}) , cpnet_vocab_path
def prune(data, cpnet_vocab_path):
    ...
    for item in tqdm(data):
        ...
        prune_qc = []
        for c in qc:
            #단어 마지막이 er이고 뒤에 두 글자를 뺀 글자가 qc에 있을 경우
...
def ground(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=1, debug=False):
    ...
    for line in lines:
        ...
        #   'image_caption':'a photography of'
        #   'image_path':'/data2/KJE/VCR/vcr1images'


        # caption.append(j["image_caption"])
        ...
        for idx,statement in enumerate(j["statements"]):

            statment_tmp.append(statement["statement"])
            if statement["label"] and statement["label"]==True:
                state_answer_idx=idx

        statment_tmp[0],statment_tmp[state_answer_idx] = statment_tmp[state_answer_idx],statment_tmp[0]

        sents.extend(statment_tmp)
        ...
        for answer in j["question"]["choices"]:
            ...
            answer_answer_idx= label_index[answer['label']]


        answers_tmp[0],answers_tmp[answer_answer_idx] = answers_tmp[answer_answer_idx],answers_tmp[0]
        answers.extend(answers_tmp)


        ##for gqa
        # statment_tmp.append(j["statements"]['statement'])
        # answers_tmp.append(j["question"]["choices"]['text'])
        #
        # sents.extend(statment_tmp)
        # answers.extend(answers_tmp)
