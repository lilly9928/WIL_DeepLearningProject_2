...
for batch_start in tqdm(range(0, len(image_urls), batch_size), desc="Evaluating"):
    ...
    for image_url, question, answer,relation in zip(batch_image_urls, batch_questions, batch_answers,batch_relations):
        ...
        prompt = (f"Please answer the question according to the\
                    above context.\
                    ===\
                    Context: a bowl , broccoli ,lemon,slices, table\
                    ===\
                    Q: How do you make that?\
                    A: steam\
                    ===\
                    Context: an orange tree,  oranges ,behind, a fence\
                    ===\
                    Q: What fruit is that?\
                    A: orange\
                    ===\
                    Context: a bowl,oranges, limes ,on a,table\
                    ===\
