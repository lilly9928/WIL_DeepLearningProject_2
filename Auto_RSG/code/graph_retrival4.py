...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(question_key_words, sentence_key_words, question):
    ...
    for qid in qc_nodes:
        if qid not in conceptnet_cache:
        ...
        for edge in q_data:
            extra_nodes.add(edge['end'])
            relations.append((edge['start'], edge['rel'], edge['end']))

    return sorted(question_key_words), sorted(sentence_key_words), question, sorted(extra_nodes), relations
...
def get_LM_score(cids, question):
    ...
    # (cid, score) 형태의 튜플 리스트로 점수와 노드 ID를 결합
    cid_score_pairs = list(zip(cids, scores))

    # 점수에 따라 내림차순으로 정렬
    sorted_cid_score_pairs = sorted(cid_score_pairs, key=lambda x: -x[1])

    # 상위 10개의 (cid, score) 튜플을 반환
    top_cid_score_pairs = sorted_cid_score_pairs[:10]

    return top_cid_score_pairs
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
    qc_ids, ac_ids, question, extra_nodes, relations = data
    # get_LM_score 함수가 (cid, score) 형태의 튜플을 원소로 가지는 리스트를 반환한다고 가정
    cid2score_list = get_LM_score(qc_ids + ac_ids + extra_nodes, question)

    # 리스트를 딕셔너리로 변환
    cid2score = {cid: score for cid, score in cid2score_list}

    # relations 데이터를 기반으로 각 노드 간 연결 강도를 평가
    relation_strength = {node: 0 for node in qc_ids + ac_ids + extra_nodes}
    for start, rel, end in relations:
        if start in relation_strength:
            relation_strength[start] += 1
        if end in relation_strength:
            relation_strength[end] += 1

    # 원래 cid2score 점수에 연결 강도를 반영하여 점수 조정
    adjusted_cid2score = {}
    for cid, score in cid2score.items():
        adjusted_score = score + relation_strength.get(cid, 0)
        adjusted_cid2score[cid] = adjusted_score

    # 조정된 점수를 기준으로 cid2score 재정렬
    adjusted_cid2score = OrderedDict(sorted(adjusted_cid2score.items(), key=lambda x: -x[1]))

    return qc_ids, ac_ids, question, extra_nodes, adjusted_cid2score, relations[:10]
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
    ...
    # relations 데이터를 기반으로 각 노드 간 연결 강도를 평가
    # 여기서는 단순화를 위해 모든 관계의 가중치를 동일하게 1로 가정
    relation_strength = {node: 0 for node in qc_ids + ac_ids + extra_nodes}
    for start, rel, end in relations:
        # 연결 강도를 가중치로 증가시킴 (여기서는 모두 1로 증가)
        if start in relation_strength:
            relation_strength[start] += 1
        if end in relation_strength:
            relation_strength[end] += 1
    ...
    adjusted_cid2score = {}
    for cid, score in cid2score.items():
        # 연결 강도에 따른 조정을 반영. 여기서는 연결 강도를 점수에 직접 더함.
        # 실제 적용에서는 이 조정 방식을 문제에 맞게 조정할 필요가 있음.
        adjusted_score = score + relation_strength.get(cid, 0)
        adjusted_cid2score[cid] = adjusted_score
    ...
    return qc_ids, ac_ids, question, extra_nodes, adjusted_cid2score, relations
"""
...
if __name__ == '__main__':
    ...
    input_path = '/data2/KJE/OK-VQA/statement/updated_OpenEnded_mscoco_val2014_questions_processed_promptcap.jsonl'
    output_path = '/data2/KJE/OK-VQA/statement/updated_OpenEnded_mscoco_val2014_questions_processed_promptcap_relation_new.jsonl'
