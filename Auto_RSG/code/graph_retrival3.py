...
#                             q_related = relation
#                             extra_nodes.add(q_related)
#
#                     for relation in s_data:
#                         if relation['start'] == sid:
#                             s_related = relation
#                             extra_nodes.add(s_related)
#
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(question_key_words,sentence_key_words,question):
    ...
    for node in qc_nodes:
        if node in conceptnet_cache:
             data = conceptnet_cache[node]
        else:
            data = get_edges_for_word(node)
            conceptnet_cache[node] = data
